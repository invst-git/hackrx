...
async def run_submission(req: RunRequest, http_request: Request):
    ...
    """
    # Optional: You can check for the bearer token if needed, but for now we'll assume it's correct
    ...
    # We use a temporary directory for downloaded files
    temp_dir = "temp_docs"
    ...
    # Use a unique namespace for each request to keep data isolated
    namespace = str(uuid.uuid4())
    
    # Generate embeddings and upsert to Pinecone in the specific namespace
    batch_size = 100
    ...
    for question in tqdm(req.questions, desc="Answering Questions"):
        # 1. Find relevant chunks from our newly indexed data
        matches = find_relevant_chunks(openai_client, pinecone_index, question, top_k=5)
        
        # 2. Generate a synthesized answer
        if matches:
